...
def prepare(*, benchmark: bool = False, M: int = None, N: int = None, eps: float = 1e-5, GROUP_SIZE_M: int = 64, dtype=np.float32):
    ...
    return {
        ...
        "DX": cp.zeros((M, N), dtype=dtype),
        "DW_partial": cp.zeros((GROUP_SIZE_M, N), dtype=np.float32),
        "DB_partial": cp.zeros((GROUP_SIZE_M, N), dtype=np.float32),
        "Locks": cp.zeros(GROUP_SIZE_M, dtype=np.int32),
        "FINAL_DW": cp.zeros(N, dtype=dtype),
