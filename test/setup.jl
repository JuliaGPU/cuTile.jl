...
## custom test record capturing GPU statistics

struct cuTileTestRecord <: AbstractTestRecord
    base::TestRecord
    gpu_bytes::UInt64
    gpu_time::Float64
...
function ParallelTestRunner.execute(::Type{cuTileTestRecord}, mod::Module, f, name,
...
function ParallelTestRunner.print_header(::Type{cuTileTestRecord}, ctx::ParallelTestRunner.TestIOContext,
...
function print_cuTile_row(io::IO, record::cuTileTestRecord, wrkr, test, ctx::ParallelTestRunner.TestIOContext;
                          color::Symbol = :white)
    base = record.base
...
function ParallelTestRunner.print_test_finished(record::cuTileTestRecord, wrkr, test,
                                                ctx::ParallelTestRunner.TestIOContext)
    lock(ctx.lock)
    try
        print_cuTile_row(ctx.stdout, record, wrkr, test, ctx; color = :white)
        flush(ctx.stdout)
    finally
        unlock(ctx.lock)
    end
...
function ParallelTestRunner.print_test_failed(record::cuTileTestRecord, wrkr, test,
                                              ctx::ParallelTestRunner.TestIOContext)
    lock(ctx.lock)
    try
        print_cuTile_row(ctx.stderr, record, wrkr, test, ctx; color = :red)
        flush(ctx.stderr)
    finally
        unlock(ctx.lock)
    end
