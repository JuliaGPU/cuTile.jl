...
# Distribution config shared by the device + host randn testsets. `edges` are
# 3 quartile-or-equivalent CDF cuts; `probs` are the matching per-bin
# probabilities; `mean` is the population mean; `in_range` is the per-element
# validity predicate.
const RANDN_DIST = (label = "randn",
                    f     = Random.randn,
                    f!    = Random.randn!,
                    edges = (-1.0, 0.0, 1.0),
...
@testset "host randn" begin
    ...
    @testset "randn! basics + counter advance" begin
        rng = ct.RNG(42)
    ...
    @testset "ct.randn / ct.randn! aliases default to Float32" begin
        ...
        @test eltype(ct.randn(Float32, N)) === Float32
