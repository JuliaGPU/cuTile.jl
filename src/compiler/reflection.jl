...
public @device_code_ptx, @device_code_sass
...
"""
    @device_code_sass [io=stdout] expression

Print the SASS machine code for all kernels compiled while evaluating the
expression, by recompiling each intercepted signature and disassembling the
result with `nvdisasm`. Works without a CUPTI subscription, so it can be used
under Nsight or another active profiler.

Note that `CUDA.@device_code_sass` also works for cuTile kernels: it
intercepts module loads at the driver level (CUPTI), showing the binary that
was actually loaded — along with Julia source locations, since `tileiras`
compiles with line info.

# Example
```julia
@device_code_sass @cuda backend=cuTile blocks=grid vadd(a, b, c)
```
"""
macro device_code_sass(ex...)
    function hook(f, tt; io::IO=stdout, kwargs...)
        println(io, "// $f($(join(tt.parameters, ", ")))")
        println(io)
        code_sass(io, f, Tuple(tt.parameters); kwargs...)
        println(io)
    end
    emit_hooked_compilation(hook, ex...)
end
