...
#
# Implemented as a `ConstAnalysis <: ForwardAnalysis{ConstLatticeElem}` on
...
"""⊤ of the constant-propagation lattice: "known non-constant / conflicting merges"."""
struct ConstTop end
const CONST_TOP = ConstTop()

"""
    ConstLatticeElem

3-state lattice for constant propagation:

    nothing      — ⊥: not yet analysed (dict-absent return value)
    Number       — known splat-constant with this scalar value
    CONST_TOP    — ⊤: known non-constant (or conflicting merges)
"""
const ConstLatticeElem = Union{Nothing, Number, ConstTop}

"""
    ConstAnalysis

Forward-propagate "this Tile is a splat of a known scalar" facts through
straight-line code and structured control flow. Recognises:
...
Everything else → `CONST_TOP`.
"""
struct ConstAnalysis <: ForwardAnalysis{ConstLatticeElem} end

bottom(::ConstAnalysis) = nothing
top(::ConstAnalysis) = CONST_TOP

function tmerge(::ConstAnalysis, a, b)
    a === nothing && return b
    b === nothing && return a
    a === CONST_TOP && return CONST_TOP
    b === CONST_TOP && return CONST_TOP
    a == b ? a : CONST_TOP
end

function operand_value(::ConstAnalysis, r::DataflowResult, @nospecialize(op))
...
function transfer(a::ConstAnalysis, r::DataflowResult, @nospecialize(func),
                  ops, block::Block, ::Any)
    if func === Intrinsics.constant && length(ops) >= 2
        v = operand_value(a, r, ops[2])
        return v isa Number ? v : CONST_TOP
    end
...
(resolved through the `ConstAnalysis` result). When called with `nothing`
(no analysis result supplied), returns the scalar only for raw literal
operands — SSAValues are treated as non-constant.
"""
function const_value(r::DataflowResult{ConstAnalysis}, @nospecialize(op))
    op isa Number && return op
    op isa QuoteNode && op.value isa Number && return op.value
    if op isa SSAValue
        v = r[op]
...
function run_passes!(sci::StructuredIRCode)
    ...
    constants = analyze(ConstAnalysis(), sci)
