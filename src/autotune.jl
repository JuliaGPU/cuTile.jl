...
# Extract hint fields (occupancy, num_ctas) from a config for cufunction kwargs.
...
# Pre-convert and compile a (cfg, args) pair into a cached TileKernel + the
# converted argument tuple ready for the call. Both the compilation and the
# adapt are reused across warmup/measurement reps.
function _prepare_launch(@nospecialize(f), cfg, args_fn::Function;
                         sm_arch::VersionNumber, opt_level::Int)
    converted = map(cuTileconvert, args_fn(cfg))
    tt = Tuple{map(Core.Typeof, converted)...}
    kernel = cufunction(f, tt; sm_arch, opt_level, hints_from_cfg(cfg)...)
    return kernel, converted
end
...
function precompile_cfg(@nospecialize(f), cfg, args_fn::Function;
    ...
    converted = map(cuTileconvert, args_fn(cfg))
    tt = Tuple{map(Core.Typeof, converted)...}
    cufunction(f, tt; sm_arch, opt_level, hints_from_cfg(cfg)...)
    return nothing
end
